...
def clustering(X_train, y_train, fit, min_cluster_size=2):
    ...
    if fit == "RL":
        for col in y_train.columns:
            ...
            coeffs[f'{col}'] = abs(model.coef_)
    elif fit == "RF":
        for col in tqdm(y_train.columns):
            model = RandomForestRegressor(n_estimators=100, random_state=42)
            model.fit(X_train, y_train[col])
            coeffs[f'{col}'] = model.feature_importances_
    elif fit == "IM":
        for col in tqdm(y_train.columns):
    ...
    # Renommer les features en X_0, X_1, ..., X_n
    new_feature_names = [f"X_{i}" for i in range(len(coeff_df.index))]
    coeff_df.index = new_feature_names


    # 👉 Centrer-réduire chaque colonne (chaque sortie)
    scaler = StandardScaler()
    coeff_scaled = scaler.fit_transform(coeff_df.values)
    coeff_df_scaled = pd.DataFrame(coeff_scaled, index=coeff_df.index, columns=coeff_df.columns)

    # Ajouter les clusters si besoin
    # clusterer = hdbscan.HDBSCAN(min_cluster_size=min_cluster_size)
    # clusters = clusterer.fit_predict(coeff_scaled)
    # coeff_df_scaled['Cluster'] = clusters

    return coeff_df_scaled.reset_index()
...
# Ajouter une colonne qui est la somme des coefficients pour chaque feature
coeff_df['Sum'] = coeff_df.iloc[:, 1:].sum(axis=1)
# Trier par la somme des coefficients
coeff_df = coeff_df.sort_values(by='Sum', ascending=False)


#Afficker une heatmap des coefficients par feature triée selon la somme des coefficients
plt.figure(figsize=(12, 4))
data_heatmap = coeff_df.iloc[:, 1:-1].T  # (features sur y)
...
ax = sns.heatmap(data_heatmap,
                 ...
                 linewidths=0.5,
                 cbar_kws={"shrink": .8},
                 yticklabels=data_heatmap.index,  # forcer noms de features affichés
                 xticklabels=False)

plt.xlabel("Variables", fontweight='bold')
plt.yticks(fontweight='bold')
cbar = ax.collections[0].colorbar
cbar.ax.tick_params(labelsize=12, labelcolor='black', width=2)
for tick in cbar.ax.get_yticklabels():
    tick.set_fontweight('bold')
plt.ylabel("")
plt.tight_layout()

plt.savefig("../../Figures/Opti/Wing/heatmap_sorties_RL.pdf", dpi=500)

plt.show()


plt.figure(figsize=(12, 6))
ax = sns.heatmap(coeff_df.iloc[:15, 1:-1].T,  # 15 premières features
            annot=True,
            cmap='coolwarm',
            fmt='.2f',
            linewidths=0.5,
...
plt.yticks(fontweight='bold')
plt.xticks(fontweight='bold')
# Récupérer les labels actuels des ticks y (axe des sorties)
yticks_labels = ax.get_xticklabels()

# Modifier les labels pour ajouter "X " devant
new_labels = ['X' + tick.get_text() for tick in yticks_labels]

# Appliquer les nouveaux labels aux ticks y
...
for tick in cbar.ax.get_yticklabels():
    tick.set_fontweight('bold')
plt.tight_layout()
plt.savefig("../../Figures/Opti/Wing/heatmap_sorties_zoom_RL.pdf", dpi=500)
...
#Afficker une heatmap des coefficients par feature triée selon la somme des coefficients
plt.figure(figsize=(12, 4))
ax = sns.heatmap(coeff_df.iloc[:, 1:-1].T,  # Transpose pour horizontale
            annot=False,
            cmap='coolwarm',
            fmt='.2f',
            linewidths=0.5,
            cbar_kws={"shrink": .8},
            yticklabels=True,  # Pas de noms de features
            xticklabels=False)  # Pas de noms de sorties non plus (optionnel)
...
for tick in cbar.ax.get_yticklabels():
    tick.set_fontweight('bold')
plt.tight_layout()
plt.savefig("../../Figures/Opti/Wing/heatmap_sorties_RF.pdf", dpi=500)
...
for tick in cbar.ax.get_yticklabels():
    tick.set_fontweight('bold')
plt.tight_layout()
plt.savefig("../../Figures/Opti/Wing/heatmap_sorties_zoom_RF.pdf", dpi=500)
plt.show()
...
for tick in cbar.ax.get_yticklabels():
    tick.set_fontweight('bold')
plt.tight_layout()
plt.savefig("../../Figures/Opti/Wing/heatmap_sorties_IM.pdf", dpi=500)
plt.show()
...
for tick in cbar.ax.get_yticklabels():
    tick.set_fontweight('bold')
plt.tight_layout()
plt.savefig("../../Figures/Opti/Wing/heatmap_sorties_zoom_IM.pdf", dpi=500)
plt.show()
